...
function createApi ({ ethWallet, tokens }) {
  function onBuyMetronome ({ password, from, value, gasLimit, gasPrice }) {
    ...
    return ethWallet.sendTransaction({
      ...
      .then(function (response) {
        analytics.event({ ec: 'Buy', ea: 'Buy MET in auction successful' })
        return response
      })
      .catch(function (err) {
        analytics.event({ ec: 'Buy', ea: 'Buy MET in auction successful failed' })
        throw err
      })
  }
  ...
  function onConvertEthToMtn ({ password, from, value, minReturn, gasLimit, gasPrice }) {
    ...
    return ethWallet.sendTransaction({
      ...
      .then(function (response) {
        analytics.event({ ec: 'Convert', ea: 'Convert ETH to MET successful' })
        return response
      })
      .catch(function (err) {
        ...
        return tokens.approveToken(
          ...
          .then(function () {
            ...
            return ethWallet.sendTransaction({
              ...
              .then(function (response) {
                analytics.event({ ec: 'Convert', ea: 'Convert MET to ETH successful' })
                return response
              })
          })
          .catch(function (err) {
